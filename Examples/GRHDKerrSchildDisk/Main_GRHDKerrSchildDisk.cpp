...
KerrSchild::params_t background_params(const SimulationConfig &config)
{
...
GRHD::CellGeometry geometry_at(double x, double y,
                               const SimulationConfig &config)
{
    KerrSchild background(background_params(config), 1.0);
...
{
    return config.torus_inner_areal_radius;
}
                   ...
                   int steps, int frames)
{
    ...
    if (!out)
        throw std::runtime_error("could not open " + path);
    out << std::setprecision(17);
    out << "driver GRHDKerrSchildDisk\n";
    out << "method fixed_grdzhadzha_kerr_schild_muscl_hlle_ssprk2\n";
...
int main()
{
    try
    {
        ...
        if (config.spin != 0.0)
        {
            throw std::runtime_error(
                "GRHDKerrSchildDisk currently assumes zero spin");
        }
        ...
        const std::string frame_dir = out_dir + "/kerr_schild_disk_frames";
        ensure_directory(frame_dir);
        const std::string base = out_dir + "/kerr_schild_disk_static";
        ...
        std::cout << "GRHD fixed Kerr-Schild disk passed..." << std::endl;
    ...
    {
        std::cout << "GRHD fixed Kerr-Schild disk failed: "
                  << error.what() << std::endl;
        return 1;
    }
    return 0;
