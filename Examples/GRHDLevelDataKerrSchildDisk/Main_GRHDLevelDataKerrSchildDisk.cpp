...
struct SimulationConfig
{
    ...
    double torus_inner_radius = 7.0;
    double torus_pressure_max_radius = 10.0;
    ...
    bool use_metric_volume_time_sources = false;
};

struct KerrSchildFactory
{
    KerrSchild::params_t params;
    double dx = 1.0;

    KerrSchild operator()(double time) const
    {
        (void)time;
...
{
    struct stat info;
    if (stat("Examples/GRHDLevelDataKerrSchildDisk", &info) == 0)
        return "Examples/GRHDLevelDataKerrSchildDisk/output";
    return "output";
}
...
KerrSchild::params_t make_background_params(
    const SimulationConfig &config)
{
    KerrSchild::params_t params;
    params.mass = config.mass;
    params.spin = config.spin;
    params.center = {{0.0, 0.0, 0.0}};
    return params;
}

KerrSchildFactory make_background_factory(const SimulationConfig &config,
                                        double dx)
{
    KerrSchildFactory factory;
...
Diagnostics compute_diagnostics(
    ...
    const KerrSchildFactory &background_factory, double time, double dx,
...
void write_slice_csv(const std::string &path, const LevelData<FArrayBox> &state,
                     const KerrSchildFactory &background_factory, double time,
                     double dx,
                     const std::array<double, CH_SPACEDIM> &center,
                     int k_mid)
{
                   ...
                   int steps, double dx)
{
    ...
    if (!out)
        throw std::runtime_error("could not open " + path);
    out << std::setprecision(17);
    out << "driver GRHDLevelDataKerrSchildDisk\n";
    out << "method fixed_grdzhadzha_kerr_schild_leveldata_constant_ell_torus_muscl_hlle_ssprk2\n";
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        synchronize_ranks();
        const std::string base = out_dir + "/leveldata_kerr_schild_disk";
        ...
        if (is_io_rank())
        {
            std::cout << "GRHD LevelData Kerr-Schild disk passed...\n"
    ...
    {
        if (is_io_rank())
            std::cerr << "GRHD LevelData Kerr-Schild disk failed: "
                      << error.what() << std::endl;
        return finish(1);
    }
